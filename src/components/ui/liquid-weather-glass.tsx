...
export const LiquidGlassCard = React.forwardRef<HTMLDivElement, LiquidGlassCardProps>(({
  ...
  const MotionComponent = (draggable || expandable ? motion.div : 'div') as React.ElementType;
  ...
  return (
    <>
      <MotionComponent
        ref={ref}
        className={cn(
          `relative ${draggable ? 'cursor-grab active:cursor-grabbing animate-none' : ''} ${expandable ? 'cursor-pointer' : ''}`,
          className
        )}
        style={{
          borderRadius,
